...
staff node が存在しない構成でも `/vstats`, `/vstats public`, `/vstats staff`, `/vstats servers`, `/vstats list`, `/vstats list staff`, `/vstats reload` は利用可能にする。
...
- 区切りは `, ` を使用する。
- 0人の server を常に表示するかどうかは、今回は Redis に存在する backend 集計に従う。将来的に常時表示が必要な場合は `backend.known-servers` を追加する。
...
void publishHeartbeat() {
    ...
    for (Player player : proxyServer.getAllPlayers()) {
        ...
        backendCounts.merge(backendName, 1, Integer::sum);
    }
...
1. `vstats.list` を確認。
2. `target=staff` の場合は `vstats.staff` も確認。
3. Redis snapshot を取得。
4. 対象プレイヤー名を抽出。staff node が存在しない場合は空リストとして扱い、node not found にはしない。
...
```text
Unknown subcommand. Usage: /vstats [public|staff|servers|list|reload]
```

`list` の使い方:

```text
Usage: /vstats list [nodeId|staff]
```
...
- `/vstats` 表示文字列生成
- `[Servers]` 表示文字列生成
- `/vstats servers` 表示文字列生成
- `/vstats public` 表示文字列生成
- `/vstats staff` 表示文字列生成
- `/vstats list` 表示文字列生成
...
/vstats list 系は whitelist コマンド風に `There are X player(s): xxx, yyy, zzzz` の形式で表示してください。`/vstats list staff` は staff node が存在しない場合でも `There are 0 player(s):` と表示してください。
...
| 6 | `/vstats list prx01` 実行 | prx01 に接続中のユーザー名だけ表示される |
| 7 | `/vstats list staff` 実行 | staff group に接続中のユーザー名だけ表示される |
| 8 | 初回起動時に `config.yml` が存在しない | `plugins/velocity-cluster-stats/config.yml` が自動生成され、Velocity 起動は継続する |
| 9 | Redis接続不可 | `/vstats` で `[stats] Redis connection error.` と表示され、Velocity のログイン・backend 移動は影響を受けない |
| 10 | Redis停止中に heartbeat が複数回失敗 | ログが cooldown され、毎回 stack trace を出さない |
| 11 | Redis復旧後 | 次回 heartbeat または command 実行で自動復帰する |
| 12 | TTL切れnodeが存在 | active nodeとして扱わない |
| 13 | `/vstats list prx99` 実行 | `Velocity node not found: prx99` と表示される |
| 14 | staff権限なしで `/vstats staff` | 権限エラーを表示する |
| 15 | `/vstats reload` 実行 | config が非同期で再読み込みされ、成功メッセージを表示する |
| 16 | Redis停止中に `/vstats reload` 実行 | Velocity のログイン・backend 移動に影響せず、Redis unavailable を表示する |
| 17 | 不正configで `/vstats reload` 実行 | reload 失敗を表示し、現在有効な設定を維持する |
| 18 | reload中に再度 `/vstats reload` 実行 | `[vstats] Reload is already in progress.` を表示する |
| 19 | Fabric Server が lobby/main/game01 の3台 | `/vstats` に `[Servers]` ブロックが表示される |
| 20 | backend server 別人数が public/staff 別に存在 | `[Servers]` に `game01: Public 9, Staff 1`, `lobby: Public 10, Staff 0`, `main: Public 23, Staff 2` が表示される |
| 21 | `/vstats servers` 実行 | `[Servers]` ブロックのみ表示される |
| 22 | server移動中のプレイヤーがいる | `unassigned: Public X, Staff Y` として `[Servers]` に表示される |
| 23 | `/vstats public` 実行 | backend server 別表示は出さず `[Public]` ブロックのみ表示される |
| 24 | `/vstats staff` 実行 | backend server 別表示は出さず `[Staff]` ブロックのみ表示される |
| 25 | staff node が存在せず public node 3台のみ | `/vstats` で `Active Velocity Nodes: 3  public=3` と表示され、`[Staff]` ブロックは表示されない |
| 26 | staff node が存在しない状態で `/vstats staff` 実行 | `[Staff]` と `staff: 0 players` が表示される |
| 27 | staff node が存在しない状態で `/vstats list staff` 実行 | `There are 0 player(s):` と表示され、`Velocity node not found` にはならない |
| 28 | staff node が存在しない状態で `/vstats servers` 実行 | 各 server 行は `serverName: Public X`、Total は `Total: X players` と表示され、`Staff 0` は表示されない |
...
- backend server が停止していても、プレイヤーがいなければ表示されない。
- 0人の backend server まで常に表示したい場合は、将来的に config に `backend.known-servers` を追加する。ただし今回は実装しない。
