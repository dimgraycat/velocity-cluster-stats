...
この Plugin は LuckPerms API へ直接依存しません。Velocity の Permission API を使うため、Velocity 対応の権限 Plugin で以下の node を制御できます。
permission が未定義の場合、コマンドはデフォルトで許可します。拒否したい場合は LuckPerms などで対象 node を `false` にしてください。
...
`vstats.staff` を明示的に拒否した場合、`/vstats list` は public player のみ表示します。staff 統計と staff player list は非表示になります。
